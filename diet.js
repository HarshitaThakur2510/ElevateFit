// Client-side Edamam Recipe Search (insecure: keys are public)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (window.location.protocol === 'file:') {
    results.innerHTML = '<p style="color:orangered">Serve this page over HTTP (use "npx http-server" or "py -3 -m http.server 3000") — running from file:// may block network requests.</p>';
  }
    ...
    try {
      ...
      hits.forEach(h => {
        ...
        card.innerHTML = `
          ...
          <h3>${r.label}</h3>
          <p><strong>Source:</strong> ${r.source} — <strong>Calories:</strong> ${Math.round(r.calories)} — <strong>Servings:</strong> ${r.yield}</p>
          <p>${(r.dietLabels||[]).join(', ')} ${(r.healthLabels||[]).slice(0,4).join(', ')}</p>
          <details><summary>Ingredients & link</summary>
